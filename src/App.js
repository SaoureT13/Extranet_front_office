...
function App() {
  ...
  const params ={userData:userData, mode:mode, apiEndpointe:apiEndpointe, paths:paths, urlBaseImage:urlBaseImage,}

  const [triggerRequest, setTriggerRequest] = useState(false);

  const handleSuccess = () => {
    console.log('Succès de la première requête, on déclenche la deuxième...');
    setTriggerRequest(true); // Déclenche la requête dans B
  };

  const handleRequestComplete = () => {
    console.log('Requête dans B terminée, réinitialisation de triggerRequest');
  ...
  return (
    <div>
      <Router basename="/extranet/">
        ...
        <TopBar param={params} triggerRequest={triggerRequest} onComplete={handleRequestComplete} />
        <Routes>
          <Route path="*" element={<Navigate to="/" />} />
          <Route path="/" element={<Accueil param={params} defaultImage={defaultImage} onSuccess={handleSuccess} />} />
          <Route path="/detail-produit" element={<DetailProduct param={params}   defaultImage={defaultImage} imageRuptureStock={imageRuptureStock} onSuccess={handleSuccess}/>} />
          <Route path="/shop" element={<Shop param={params} defaultImage={defaultImage}/>} />
          <Route path="/cart" element={<Cart   param={params} onSuccess={handleSuccess}/>} />
          <Route path="/form" element={<Form />} />
          <Route path="/contact" element={<Contact />} />
          <Route path="/choix-livraison" element={<Livraison onSuccess={handleSuccess}/>} />
          <Route path="/partenaire" element={<PartnerList />} />
          <Route path="/saveclient" element={<SaveClient />} />
          <Route path="/livraison" element={<DeliverySchedule param={params} />} />
